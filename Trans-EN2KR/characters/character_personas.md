# OPUS: 별노래의 메아리 — 인물 페르소나 참고표 (대사 번역용)

> 화자 매핑 추출 결과(4728 대사 중 4515=95% 화자 식별 완료)에서 도출.
> **성별 + 성격 영어 태그**는 4개 언어(EN/CHT) 실제 대사 + 역할(locKey) + 작품 설정 기반 분석.
> 외부 AI 페르소나 후보 조회용. `(?)` = 성별/설정 확인 필요.
...
| KR | 원형 | CHT | 성별 | 대사수 | 성격 영어 태그 |
|---|---|---|---|---:|---|
| 에다(소녀) | Eda(teen) | 艾妲少女 | F | 89 | `earnest` `aspiring` `vulnerable` `determined` `dreamy` |
| 에다(어린시절) | Eda(child) | 艾妲小時候 | F | 63 | `frail` `sickly` `lonely` `hopeful` `innocent` |
| 준(노년) | Jun(elder) | 李莫老年 | M | 86 | `melancholic` `reflective` `wistful` `regretful` `weary`  ※프레임 화자(회상) |
| 준(어린시절) | Jun(child) | 李莫小時候 | M | 44 | `stoic` `precocious` `dutiful` `gentle` `self-controlled` |
| 레미(어린시절) | Remi(child) | 菈米亞小時候 | F | 50 | `streetwise-orphan` `wary` `blunt` `survival-minded` |

## 조·단역 (성격 태그 간략)

| KR | CHT | 성별 | 대사수 | 태그 |
|---|---|---|---:|---|
| 광련 병사 A/B | 礦聯士兵 | M | 36/9 | `officious` `procedural` `dismissive` |
| 백방 도적 B/C | 白幫盜賊 | M | 26/3 | `rough` `jittery` `subordinate-thug` |
| 무녀탑 직원 | 巫塔人員 | ?(중립) | 23 | `bureaucratic` `cold` `procedural` |
| 조산 십인회 이사 A~C | 朝山十人會理事 | M | 19/4/6 | `formal` `guarded` `mercantile-elder` |
